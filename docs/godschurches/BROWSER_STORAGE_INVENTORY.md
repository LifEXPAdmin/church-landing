...
verified live in 2026.09.26.5. Saved-draft read cleanup and exact discard recovery
are verified in 2026.09.26.6. Read-only Support list cleanup is verified in
2026.09.26.7. Get help form DOM cleanup with retained in-memory recovery is
verified in 2026.09.26.8; case detail, routing and attachment presentation cleanup
...
| `draft-library.tsx` | Private list rows are client-read only. Current-access reads conceal them; failures, blur, hidden/pagehide and offline clear them. Generation checks reject late responses. The exact unconfirmed discard command remains owner-bound through failed reads and refresh. No Web Storage is used. Final browser acceptance passed with account replacement, held reads and unchanged tombstone/version after retry. |
| `support-form.tsx`, `private-snapshot-guard.tsx`, `community-report-form.tsx`, `community-report-review.tsx` | Support owns its serialized retry body in the mounted form. Get help opts into controlled in-memory values and removes its private DOM controls during concealment, preserving the frozen original context and uncertain command. Its 429 retry preserves the original key. Support detail/routing use the same controlled memory with frozen command owners and sibling-save protection. Feedback receipt detail also removes private controls, retaining controlled choice drafts and original command owners in memory. Compatible current versions require deliberate adoption for dirty entries; structural command removal stays concealed while work remains. Feedback intake, Admin and other non-opt-in forms retain their existing DOM owners. Report/review owners conditionally remove private rendered fields while retaining owner-bound pending requests in memory. Guarded server snapshots and ordinary/reposted readers can remain in hidden/inert DOM. A status-only global unmount would destroy Support recovery. The shared read-only snapshot owner instead loads scheduled and Support requests/inbox DTOs after hydration and clears rendered data on concealment. Late responses are rejected; changed snapshots require deliberate reload. Support list pagination uses fresh documents. Scoped Support list, new-form, detail/routing and attachment presentation acceptance passed; other guarded readers and form callers remain open. |
...
- Scheduled-list DOM/serialized-page and saved-draft failed-read cleanup passed.
  Exact draft discard recovery remains intact. Continue the broader
