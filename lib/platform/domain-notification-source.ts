...
export async function domainNotificationSources(
  ...
  const groups = await groupNotificationSources(
    tx,
    events.filter(
      (e) => e.kind === "GROUP_MEMBERSHIP" || e.kind === "GROUP_REVIEW"
    ),
    context,
    feedbackChannel
  );
  for (const [id, source] of groups) result.set(id, source);
  const assistance = await pantryNotificationSources(
    tx,
    events.filter((e) => e.kind === "PANTRY_REQUEST"),
    context,
    feedbackChannel
  );
  ...
  const exchange = await exchangeNotificationSources(
    tx,
    events.filter((event) => event.kind === "EXCHANGE_MATCH"),
    context,
    feedbackChannel
  );
  for (const [id, source] of exchange) result.set(id, source);
