...
ALTER TABLE "GatherGroup" ADD CONSTRAINT "GatherGroup_shape" CHECK (
  ...
  (discovery <> 'UNLISTED' OR "joinPolicy"='INVITE_ONLY') AND
...
-- Independent, dated group phone opt-in; generic canonical activity only.
...
ALTER TABLE "SocialPreferences" ADD CONSTRAINT "SocialPreferences_notification_shape_check" CHECK (
  "notificationVersion" >= 0 AND cardinality("mutedNotificationCategories") <= 19
  AND "mutedNotificationCategories" <@ ARRAY['messages','requests','reports','founder','replies','mentions','conversations','prayer','posts','reactions','church','commitments','feedback','photos','exchange','handoffs','needs','assistance','groups']::TEXT[]
...
ALTER TABLE "SocialEvent" ADD CONSTRAINT "SocialEvent_notification_source_check" CHECK (
  ...
  AND ("notificationCategory" IS NULL OR "notificationCategory" IN ('messages','requests','reports','founder','replies','mentions','conversations','prayer','posts','reactions','church','commitments','feedback','photos','exchange','handoffs','needs','assistance','groups')));
...
ALTER TABLE "SocialPreferences" ADD CONSTRAINT notification_choices CHECK (
  "pushCategories" <@ ARRAY['messages','requests','reports','founder','replies','mentions','conversations','prayer','posts','reactions','church','commitments','feedback','photos','exchange','handoffs','needs','assistance','groups']::text[] AND
