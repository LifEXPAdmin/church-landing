...
-- Retain previous publication receipts while accepting explicit equipment-loan terms.
...
ALTER TABLE "ExchangeListing" ADD CONSTRAINT "ExchangeListing_shape" CHECK (
  ...
  AND (state NOT IN ('ACTIVE','RESERVED','CLOSED') OR (
    ...
    AND ("itemPolicy" IN ('exchange-listings-v2','exchange-listings-v3') OR (intent::text IN ('FREE','SALE') AND "itemPolicy"='ordinary-items-v1'))
