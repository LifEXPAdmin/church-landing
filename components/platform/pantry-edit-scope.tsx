...
// A confirmed pantry save refreshes versioned sibling forms. Finish one section
// before editing another so that refresh cannot discard independent local work.
export function PantryEditScope({ children }: { children: ReactNode }) {
...
export function usePantryEdit(label: string) {
  const scope = useContext(Context),
    id = useId();
  if (!scope) throw new Error("Pantry editing requires its page scope");
